...
static int __cxl_hmu_start(struct perf_event *event, int flags)
{
	 ...
	 */
	if (hmu->irq <= 0) {
		if (!is_sampling_event(hmu->event))
			return -EINVAL;
		goto output_begin;
	}
...
output_begin:
	 ...
	 */
	status = readq(hmu->base + CHMU_INST0_STATUS_REG);
	if (hmu->hot_thresh > (1 << FIELD_GET(CHMU_INST0_STATUS_COUNTER_WIDTH_MSK, status)))
		return -EINVAL;
	/* Start the unit up */
	val = FIELD_PREP(CHMU_INST0_CFG0_WHAT_MSK, hmu->m2s_requests_to_track) |
		...
		FIELD_PREP(CHMU_INST0_CFG0_OVRFLW_INT_EN, hmu->irq <= 0 ? 0 : 1) |
		FIELD_PREP(CHMU_INST0_CFG0_FILLTHRESH_INT_EN, hmu->irq <= 0 ? 0 : 1) |
	...
	if (hmu->irq <= 0) {
		...
		period = hmu->event->attr.freq ? (NSEC_PER_SEC / hmu->event->attr.sample_freq) :
			hmu->event->attr.sample_period;
		period = max_t(u64, 10000, period);
		hmu->interval = ns_to_ktime(period);
		hrtimer_start(&hmu->hrtimer, hmu->interval, HRTIMER_MODE_REL_PINNED);
	}
...
static int probe_phys_addr(struct device *dev, void *data)
{
	struct cxl_hmu *hmu = data;
	struct pci_dev *pdev = to_pci_dev(hmu->dev.parent);

	if (is_cxl_memdev(dev)) {
		struct cxl_memdev *cxlmd = to_cxl_memdev(dev);

		if (pdev->bus->bridge == cxlmd->endpoint->parent_dport->dport_dev) {
			unsigned long index;
			struct cxl_region_ref *iter;

			xa_for_each(&cxlmd->endpoint->regions, index, iter) {
				hmu->phys_addr_start = iter->decoder->hpa_range.start;
				return 1;
			}
		}
	}
...
static enum hrtimer_restart cxl_hmu_pci_hrtimer_handler(struct hrtimer *hrtimer)
{
	struct cxl_hmu_info *hmu = container_of(hrtimer, struct cxl_hmu_info, hrtimer);
	u64 offset = readq(hmu->base + CHMU_INST0_HOTLIST_OFFSET_REG);
	u16 head = readw(hmu->base + CHMU_INST0_HEAD_REG);
	u16 tail = readw(hmu->base + CHMU_INST0_TAIL_REG);
	u8 count_width = FIELD_GET(CHMU_INST0_STATUS_COUNTER_WIDTH_MSK,
	...
	perf_sample_data_init(&data, 0, hmu->event->hw.last_period);
	data.sample_flags |= PERF_SAMPLE_PHYS_ADDR;

	while (head != tail && !overflow) {
		const u64 phys_addr_mask = (1ULL << 36) - 1;

		data.phys_addr = readq(hmu->base + offset + head * 8);
		data.phys_addr &= ~((1ULL << count_width) - 1);
		data.phys_addr &= phys_addr_mask;
		if (data.phys_addr < (hmu->phys_addr_start & phys_addr_mask))
			data.phys_addr += 1ULL << 36;
		data.phys_addr += hmu->phys_addr_start & ~phys_addr_mask;

		overflow = perf_event_overflow(hmu->event, &data, regs);
		delta++;
		head = (head + 1) % top;
	}
	...
	//hmu->event->hw.last_period = hmu->event->hw.sample_period;
	local64_add(delta, &hmu->event->count);
	local64_sub(delta, &hmu->event->hw.period_left);

	if (hmu->event->hw.state & PERF_HES_STOPPED)
		return HRTIMER_NORESTART;

	hrtimer_forward_now(&hmu->hrtimer, hmu->interval);
...
static int cxl_hmu_probe(struct device *dev)
{
	...
	bus_for_each_dev(&cxl_bus_type, NULL, hmu, probe_phys_addr);
	if (WARN_ON(hmu->phys_addr_start == 0))
		return -ENODEV;
	...
	for (i = 0; i < num_inst; i++) {
		...
		info->on_cpu = -1;
		info->phys_addr_start = hmu->phys_addr_start;
		...
		info->irq = rc;
		if (rc <= 0) {
			hrtimer_setup(&info->hrtimer, cxl_hmu_pci_hrtimer_handler, CLOCK_MONOTONIC,
					HRTIMER_MODE_REL);
			info->on_cpu = info->hrtimer.base->cpu_base->cpu;
			goto pmu_register;
		}
...
static DEFINE_IDA(cxl_hmu_ida);

static void cxl_hmu_release(struct device *dev)
{
	struct cxl_hmu *hmu = to_cxl_hmu(dev);

	ida_free(&cxl_hmu_ida, hmu->assoc_id);
}
...
static int cxl_hmu_pci_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	struct cxl_hmu *hmu;
	...
	hmu = devm_kzalloc(&pdev->dev, sizeof(*hmu), GFP_KERNEL);
	if (!hmu)
		return -ENOMEM;
	...
	hmu->base = pcim_iomap_region(pdev, 2, "cxl_hmu_pci");
	if (IS_ERR(hmu->base)) {
		rc = PTR_ERR(hmu->base);
		pci_err(pdev, "failed to request pci region: %d\n", rc);
		return rc;
	}

	if (pci_resource_len(pdev, 2) != 2 * 1024 * 1024)
		return -ENODEV;

	rc = ida_alloc(&cxl_hmu_ida, GFP_KERNEL);
	if (rc < 0)
		return rc;
	...
	dev->parent = parent;
	dev->release = cxl_hmu_release;
	rc = dev_set_name(dev, "hmu_mem%d.%d", hmu->assoc_id, hmu->index);
	if (rc)
		goto err;
	...
	rc = devm_add_action_or_reset(parent, remove_dev, dev);
	if (rc)
		goto err;

	return cxl_hmu_probe(dev);
...
static const struct pci_device_id cxl_hmu_pci_ids[] = {
	{ PCI_DEVICE(PCI_VENDOR_ID_INTEL, 0x0ddb) },
	{ 0, }
};

static struct pci_driver cxl_hmu_pci_driver = {
	.name = "cxl_hmu_pci",
	.id_table = cxl_hmu_pci_ids,
	.probe = cxl_hmu_pci_probe,
};
...
static __init int cxl_hmu_init(void)
{
	...
	rc = pci_register_driver(&cxl_hmu_pci_driver);
	if (rc) {
...
{
	pci_unregister_driver(&cxl_hmu_pci_driver);
	cxl_driver_unregister(&cxl_hmu_driver);
	cpuhp_remove_multi_state(cxl_hmu_cpuhp_state_num);
}
