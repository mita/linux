...
static int hmem_probe_phys_addr(struct device *dev, int target_nid, const struct resource *res)
{
	struct cxl_hmu *hmu = to_cxl_hmu(dev);

	if (hmu->nid == target_nid)
		hmu->phys_addr_start = res->start;
...
static int cxl_hmu_probe(struct device *dev)
{
	...
	walk_hmem_resources(dev, hmem_probe_phys_addr);
	if (hmu->phys_addr_start == 0)
		return -ENODEV;
